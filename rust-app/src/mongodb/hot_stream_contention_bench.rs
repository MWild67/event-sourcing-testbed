...
impl HotStreamContentionResult {
    pub fn print_report(&self) {
        println!();
        println!("══════════════════════════════════════════════");
        println!("  MongoDB Hot-Stream Contention Result");
        println!("══════════════════════════════════════════════");
        println!(
...
pub async fn run(
    ...
    info!(
        ...
        "starting MongoDB hot-stream contention benchmark"
    );
    ...
    if !ready {
        anyhow::bail!("MongoDB did not become ready within 45 s");
    }
    ...
    for stream in &all_streams {
        client.ensure_collection_event_store(stream).await?;
    }
    client.init_event_store_counters(&all_streams).await?;
        ...
        tokio::spawn(async move {
            ...
            if is_hot {
                ...
                for attempt in 0..=max_retries {
                    ...
                    let result = client
                        .append_with_stream_version(&stream_name, "BenchmarkEvent", &ev, expected)
                        .await;
                ...
                let t0 = Instant::now();
                match client
                    .append_batch_versioned(&stream_name, "BenchmarkEvent", &[ev])
                    .await
                {
                    Ok(()) => {
