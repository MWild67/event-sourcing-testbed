...
impl HotStreamContentionResult {
    pub fn print_report(&self) {
        println!();
        println!("══════════════════════════════════════════════");
        println!("  PostgreSQL Hot-Stream Contention Result");
        println!("══════════════════════════════════════════════");
        println!(
...
pub async fn run(
    ...
    info!(
        ...
        "starting PostgreSQL hot-stream contention benchmark"
    );
    ...
    if !ready {
        anyhow::bail!("PostgreSQL did not become ready within 45 s");
    }
    ...
    let all_streams: Vec<String> = (0..(hot_streams + cold_streams))
        .map(|i| format!("{}-stream-{}", config.stream_prefix, i))
        .collect();
    client.init_stream_versions(&all_streams).await?;
    ...
    let baseline =
        run_baseline_phase(Arc::clone(&client), &config, hot_streams, cold_streams).await?;
    ...
    while wall_start.elapsed() < phase_duration {
        ...
        tokio::spawn(async move {
            ...
            if is_hot {
                ...
                let stream_name = format!("{}-stream-{}", prefix, hot_idx);
                ...
                for attempt in 0..=max_retries {
                    let expected = hot_rev.load(Ordering::Acquire) as i64;
                    let result = client
                        .append_with_stream_version(
                            &stream_name,
                            "BenchmarkEvent",
                            &ev,
                            expected,
                    ...
                    match result {
                        ...
                        Err(err) => {
                            ...
                            let is_conflict = msg.contains("wrong expected version")
                            ...
                            if is_conflict {
                                ...
                                if let Ok(Some(current)) =
                                    client.read_last_stream_version(&stream_name).await
                                {
                                    hot_rev.store((current + 1) as u64, Ordering::Release);
                                }
                                continue;
                            }
                ...
                let stream_name = format!("{}-stream-{}", prefix, hot_streams + cold_idx);
...
async fn run_baseline_phase(
    ...
    while wall_start.elapsed() < phase_duration {
        ...
        let prefix = config.stream_prefix.clone();
        let stream_count = hot_streams + cold_streams;
        ...
        tokio::spawn(async move {
            let _permit = permit;
            let stream_idx = seq_now % stream_count;
            let stream_name = format!("{}-stream-{}", prefix, stream_idx);
            let ev = BenchmarkEvent::new(seq_now, stream_idx);
            let t0 = Instant::now();

            match client
                .append_batch_versioned(&stream_name, "BenchmarkEvent", &[ev], seq_now, stream_idx)
                .await
            {
                Ok(_) => {
                    let lat_us = u64::try_from(t0.elapsed().as_micros()).unwrap_or(u64::MAX);
                ...
                Err(e) => {
                    warn!(error = %e, stream = %stream_name, "baseline write failed");
                }
            }
        });
    }
